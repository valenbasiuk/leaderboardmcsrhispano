 ...
 */
export function getSeasonFromTimestamp(timestamp: number): number | undefined {
...
/**
 * Parses user detailed statistics to extract best times
 */
export function parseProfileToRunner(runner: Runner, userData: any): Runner | null {
...
export interface TimeLeaderboardData {
    initialData: Runner[]
    allRunners: Runner[]
}
 ...
 * Steps:
 * 1. Fetch all Hispanic runners (via Elo leaderboard)
 * 2. Fetch detailed profile for Top 30 runners (initial server-side payload)
 * 3. Return both list of all runners and the initial detailed data
 */
export default async function getHispanicTimeLeaderboard(): Promise<TimeLeaderboardData> {
    try {
        // 1. Get the base list of Hispanic runners
        const eloRunners = await getHispanicLeaderboard()

        // 2. Limit to Top 30 to avoid rate limits and keep page load fast
        const topRunners = eloRunners.slice(0, 30)

        // 3. Fetch full profile for initial runners in parallel with CACHING
        const profilePromises = topRunners.map(async (runner): Promise<Runner | null> => {
            try {
                ...
                return parseProfileToRunner(runner, response.data)
            } catch (error) {
        ...
        const initialData = detailedRunners.filter((r): r is Runner => r !== null)

        return {
            initialData,
            allRunners: eloRunners,
        }
    } catch (error) {
